...
const DataSources = {
  ...
  async fromChartMock() {
    return await ajaxGetJson(
      '/static/mock_userdata.json',
      'Failed to load mock chart data',
    );
  },
};
...
const renderProfile = (profile) => {
  ...
  setText(
    'stat-successful-transactions',
    profile.user.stats.successfulTransactions,
  );
  setText(
    'stat-failed-transactions',
    Math.max(
      profile.user.stats.totalTransactions -
        profile.user.stats.successfulTransactions,
      0,
    ),
  );
...
const renderCharts = (profile) => {
  ...
  toggleChartPlaceholder(
    'postsCommentsBarChart',
    'placeholder-posts-comments',
    !hasPostsCommentsData,
  );
  ...
  const hasTotalBuyingData =
    totalBuyingLabels.length > 0 &&
    totalBuyingValues.some((value) => value > 0);
  toggleChartPlaceholder(
    'totalBuyingLineChart',
  ...
  const successful =
    profile.charts.transactionOutcome?.successful ??
    profile.user.stats.successfulTransactions;
  const failed =
    profile.charts.transactionOutcome?.failed ??
    Math.max(profile.user.stats.totalTransactions - successful, 0);
  const hasOutcomeData = successful + failed > 0;
  toggleChartPlaceholder(
    'transactionOutcomePieChart',
    'placeholder-outcome',
    !hasOutcomeData,
  );
